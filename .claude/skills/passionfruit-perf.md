...
description: Use when editing any media component (`<Image>`), `BaseLayout.astro`, files under `src/pages/`, or adding new client scripts. Loads image-loading discipline, no-JS-when-CSS-suffices, third-party host CSP rules.
---
...
3. **Heavy third-party embeds use facade components.** Video players, podcast players, maps, and similar heavy embeds go through a facade. Never embed the third-party iframe directly — facades respect cookie consent and defer the real embed until the user interacts. Implement any new facade in `src/components/` following the pattern: show a static preview, load the real embed on user interaction.
...
- **Third-party iframe embedded directly** — bloats LCP by hundreds of kilobytes, ignores cookie consent, drags in third-party JS before the user has shown intent. Use a facade.
