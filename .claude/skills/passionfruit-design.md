...
- **Need to display a blog entry as a card?** Use `BlogCard`. passionfruit deliberately has one card per content type, not a generic frame.
- **Need to embed a third-party video or audio player?** Use a facade pattern — never embed the third-party iframe directly. Implement a facade component in `src/components/` that defers the real embed until user interaction and respects cookie consent.
- **Need long-form Markdown rendering?** Reuse the existing `BlogPost` / `PageContent` patterns.
...
- **Need to add a translation string?** Update both `src/i18n/de.json` and `src/i18n/en.json` together (see CLAUDE.md §5 and §6 for the bilingual rule and CONTRIBUTING.md for the full workflow).
...
- **CLAUDE.md §5 and §6** — bilingual rule and content workflows (translationKey, i18n JSON lockstep, CONTRIBUTING.md).
